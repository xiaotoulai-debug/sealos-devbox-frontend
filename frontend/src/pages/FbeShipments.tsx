...
export default function FbeShipments() {
  ...
  const openDetail = useCallback(async (record: FbeShipment) => {
    setDrawerOpen(true);
    if (record.items?.length) { setDetail(record); return; }
    ...
    try {
      ...
      else message.error(res.message || '加载明细失败');
  ...
  return (
    <div>
      ...
      <DetailDrawer
        ...
        onSaved={() => {
          // 保存数量后重新拉取明细，同时刷新列表
          if (detail) openDetail({ ...detail, items: undefined });
          refresh();
        }}
      />
...
function DetailDrawer({ open, loading, detail, onClose, onSaved }: DetailDrawerProps) {
  ...
  const hasChange = detail?.items?.some((it) => qtyMap[it.id] !== undefined && qtyMap[it.id] !== it.quantity) ?? false;

  const handleSaveQty = async () => {
    if (!detail) return;
    const changed = (detail.items ?? [])
      .filter((it) => qtyMap[it.id] !== undefined && qtyMap[it.id] !== it.quantity)
      .map((it) => ({ itemId: it.id, quantity: qtyMap[it.id] }));
    if (changed.length === 0) { message.info('没有数量变更'); return; }
    setSaving(true);
    try {
      const { data: res } = await request.put<{ code: number; message: string }>(
        `/fbe-shipments/${detail.id}/items`,
        { items: changed },
      );
      if (res.code === 200) {
        message.success(res.message || '数量已保存');
        onSaved?.();
      } else {
            ...
            columns={[
              {
                ...
                render: (_: unknown, r: FbeShipmentItem) => {
                  const src = pickImageUrl(r.product) ?? pickImageUrl(r) ?? '';
                  return (
                    <Image
                      src={src || undefined}
                ...
                render: (_: unknown, r: FbeShipmentItem) => {
                  ...
                  return (
                    <div style={{ minWidth: 0 }}>
                      <Text code type="secondary" style={{ fontSize: 12 }}>{sku}</Text>
                      <Text ellipsis style={{ display: 'block', maxWidth: 240, fontSize: 13 }}>{name}</Text>
                    </div>
                  );
                },
              },
...
function buildSearchLabelPlain(p: StoreProductPick): string {
  ...
  return [sku, ean, pnk, name, cn].filter(Boolean).join(' ');
}
...
function ManualCreateFbeShipmentModal({
  ...
  const addLine = useCallback((product: StoreProductPick) => {
    ...
    setLines((prev) => {
      if (prev.some((r) => r.productId === pid)) {
      ...
      return [...prev, {
        ...
        name:            product.title ?? product.name ?? product.product_name ?? product.productName ?? '-',
  ...
  const updateQty = useCallback((productId: number, v: number | null) => {
    setLines((prev) => prev.map((r) => (r.productId === productId ? { ...r, quantity: v ?? 1 } : r)));
  }, []);

  const removeLine = useCallback((productId: number) => {
    setLines((prev) => prev.filter((r) => r.productId !== productId));
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    // 防超发：有仓库选择时校验不能超过该仓可用库存
    if (values.warehouseId) {
      const overstock = lines.find((r) => {
        const avail = getAvailableStock(r.warehouseStocks, values.warehouseId);
        return avail !== null && r.quantity > avail;
      });
      if (overstock) {
        const avail = getAvailableStock(overstock.warehouseStocks, values.warehouseId) ?? 0;
        message.error(`SKU [${overstock.sku}] 发货数量 (${overstock.quantity}) 超过该仓可用库存 (${avail})，请调整后重试`);
        return;
      }
    }
    setSubmitting(true);
      ...
      <div style={{ marginTop: 8 }}>
          ...
          <>
            <Select
              ...
              onSelect={(value: number) => {
                const found = productOptions.find((o) => o.value === value);
        ...
        <Table<ManualLineRow>
          ...
          columns={[
            ...
            { title: 'SKU', dataIndex: 'sku', width: 130, ellipsis: true, render: (v: string) => <Text code style={{ fontSize: 12 }}>{v}</Text> },
            ...
            },
            {
              ...
              render: (_: unknown, r: ManualLineRow) => {
                const avail = getAvailableStock(r.warehouseStocks, warehouseIdWatch);
                const exceeded = avail !== null && r.quantity > avail;
                return (
                  <InputNumber
                    min={1}
                    max={avail ?? undefined}
                    precision={0}
                    value={r.quantity}
                    style={{ width: 100, borderColor: exceeded ? '#ef4444' : undefined }}
                    status={exceeded ? 'error' : undefined}
                    onChange={(v) => updateQty(r.productId, v)}
                  />
                );
              },
            },
            {
              ...
              align: 'center',
              render: (_: unknown, r: ManualLineRow) => (
                <Button type="link" danger size="small" onClick={() => removeLine(r.productId)}>删除</Button>
              ),
            },
          ]}
        />
...
export function CreateFbeShipmentModal({ open, shopId, products, onCancel, onSuccess }: CreateFbeShipmentModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    // 防超发校验
    const overstock = rows.find((r) => {
      const avail = getAvailableStock(r.warehouseStocks, warehouseId);
      return avail !== null && r.quantity > avail;
    });
    if (overstock) {
      const avail = getAvailableStock(overstock.warehouseStocks, warehouseId) ?? 0;
      message.error(`SKU [${overstock.sku}] 发货数量 (${overstock.quantity}) 超过该仓可用库存 (${avail})`);
      return;
    }
        ...
        columns={[
          {
            ...
            render: (_: unknown, r: CreateShipmentRow) => (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                />
                <div style={{ minWidth: 0 }}>
                  <Text ellipsis style={{ display: 'block', maxWidth: 210 }}>{r.name}</Text>
                  <Text code type="secondary" style={{ fontSize: 12 }}>{r.sku}</Text>
                </div>
              </div>
            ),
          },
            ...
            render: (_: unknown, r: CreateShipmentRow) => {
              ...
              return (
                <InputNumber
                  ...
                  precision={0}
                  size="small"
                  style={{ width: 90, borderColor: exceeded ? '#ef4444' : undefined }}
                  status={exceeded ? 'error' : undefined}
                  suffix="件"
