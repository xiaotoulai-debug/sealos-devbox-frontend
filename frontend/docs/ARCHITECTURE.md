...
#### 2.2.3 采购单链路「归属店铺」打标设计

- **交互入口**: 采用「末端打标」策略。不在商品加入采购计划时强制选择店铺，而是在最终的 `OrderConfirmModal`（核对并创建采购单）弹窗中提供「归属店铺（可选）」下拉选择，保留业务员跨商品拼单与通用备货的灵活性。
- **链路透传**: 平台产品返单进入采购计划时，当前页面的 `shopId` 会随 `RepeatPurchaseModal` 提交到后端；采购计划批量创建采购单时，前端根据 `selectedRows` 的 `shopId` 做单店一致性推断，仅当所有选中行都有且只有同一个 `shopId` 时自动填充弹窗默认店铺。
- **列表兼容性渲染**: `ProcurementManagement` 采购管理列表对历史老数据做防御性兜底。存在 `shopId` 或店铺快照名称时，采购单编号下方渲染彩色专属店铺 Tag；为 `null` 或无店铺信息时，渲染灰色 `<Tag color="default">通用备货</Tag>`，避免旧采购单显示异常。

#### 2.2.4 采购建议 Popover 与库存健康度标签

- **展示策略**: `PlatformProducts` 平台产品列表在「平台库存」之后增加「采购建议」列，采用高信息密度的 Ant Design `Popover`。主文本显示「需补货 X」或「安全」，Hover 时展示完整算账明细，包括目标水位（日销 * 60）以及平台可用库存、平台在途、本地备货仓、1688 采购在途、已入采购计划等扣减量，确保补货逻辑透明可追溯。
- **防御性取值**: 对后端 DTO 拼装可能遗漏的明细字段，前端使用双空值合并运算符做跨层级降级取值，例如 `suggestion?.platformInTransit ?? root.in_transit_quantity ?? 0`。禁止使用 `||` 处理库存数量，避免合法的 `0` 被误判为缺失。
- **零入侵状态标签排版**: 为避免新增 Table Column 导致横向滚动变长，「采购建议」列内部使用 `<Space direction="vertical">` 将建议文本与状态标签上下叠放，不增加新的列宽负担。
- **库存健康度映射**:
  - `NEW` → `<Tag color="cyan">✨ 新品</Tag>`，用于新品 30 天免责识别。
  - `DEAD` → `<Tag color="red">🔴 滞销款</Tag>`，用于极高优先级处理。
  - `HOT` → `<Tag color="orange">🔥 热销款</Tag>`，用于防断货提醒。
  - `NORMAL` → 隐式处理，不渲染额外标签，减少视觉噪点。
