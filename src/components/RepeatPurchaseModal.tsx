...
export default function RepeatPurchaseModal({ open, rows, shopId, onCancel, onSuccess }: RepeatPurchaseModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const { data: res } = await request.put<{ code: number; message: string; data: { count: number } }>(
        '/products/batch-to-purchasing', { items, shopId: shopId ?? undefined },
      );
      if (res.code === 200) {
