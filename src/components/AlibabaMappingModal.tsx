...
interface AlibabaBindDonePayload {
  action: 'bind' | 'unbind';
  productId: number;
  offerId?: string;
  specId?: string;
...
interface AlibabaMappingModalProps {
  ...
  onCancel: () => void;
  onDone: (payload?: AlibabaBindDonePayload) => void;
}
...
export default function AlibabaMappingModal({
  ...
  const handleBind = useCallback(async () => {
    setBinding(true);
    try {
      // productId 必须由父组件（采购单明细接口）提供，前端不做任何猜测性兜底
      if (!productId) {
        message.warning('当前采购明细缺少主产品 ID，暂无法绑定');
        return;
      }
      if (!selectedSkuId) {
      ...
      const selectedItem = Array.isArray(parsedSkus)
        ? parsedSkus.find((s) => s.skuId === selectedSkuId)
        : null;
      const specIdToSend = selectedItem?.specId ?? selectedSkuId;
      const skuIdToSend  = selectedItem?.skuId  ?? selectedSkuId;

      if (!specIdToSend) {
        message.warning('所选规格缺少 specId（32 位哈希），无法完成绑定');
        return;
      }

      const offerIdToSend = offerId ?? currentOfferId ?? undefined;

      const { data: res } = await request.put<{ code: number; message: string }>(
        '/alibaba/bind',
        { productId, offerId: offerIdToSend, specId: specIdToSend, skuId: skuIdToSend },
      );

      if (res.code === 200) {
        message.success(res.message || '1688 规格绑定成功');
        onDone({
          action: 'bind',
          productId,
        ...
        message.error(res.message || '绑定失败，请重试');
      }
    } catch (err: unknown) {
      const resp = (err as { response?: { data?: { message?: string } } })?.response?.data;
      message.error(resp?.message ?? '绑定请求失败，请稍后重试');
    } finally {
      setBinding(false);
    }
  }, [productId, offerId, currentOfferId, selectedSkuId, parsedSkus, onDone]);

  const handleUnbind = useCallback(async () => {
    if (!productId) { message.error('系统数据不完整，无法解除绑定'); return; }
    setUnbinding(true);
    try {
      const { data: res } = await request.put<{ code: number; message: string }>(
      ...
      if (res.code === 200) {
        ...
        onDone({ action: 'unbind', productId });
  ...
  const missingProductId = productId == null;
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0' }}>
          ...
          <Space size={12}>
            ...
            <Button
              ...
              loading={binding}
              onClick={handleBind}
              disabled={missingProductId || !selectedSkuId}
              style={missingProductId || !selectedSkuId ? undefined : { background: '#ff6a00', borderColor: '#ff6a00' }}
            >
              确认绑定
            </Button>
          </Space>
    ...
    >
      {missingProductId && (
        <Alert
          ...
          message="无法完成规格绑定"
          description="当前采购明细缺少主产品 ID，请先联系后端补齐字段或刷新重试"
        />
      )}
