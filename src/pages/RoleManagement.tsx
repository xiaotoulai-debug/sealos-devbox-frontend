...
function buildPermissionTreeMeta(nodes: PermTreeNode[]): PermissionTreeMeta {
  const groupKeys = new Set<string>();
  const parentByKey = new Map<string, string>();
  const descendantsByKey = new Map<string, string[]>();

  const walk = (items: PermTreeNode[], parentKey?: string): string[] => {
    const collected: string[] = [];
    for (const item of items) {
      if (item.isGroup) groupKeys.add(item.key);
      if (parentKey) parentByKey.set(item.key, parentKey);

      const childDescendants = item.children ? walk(item.children, item.key) : [];
      descendantsByKey.set(item.key, childDescendants);
      collected.push(item.key, ...childDescendants);
    }
    return collected;
  };

  walk(nodes);
  return { groupKeys, parentByKey, descendantsByKey };
}

function getPermissionAncestors(key: string, meta: PermissionTreeMeta): string[] {
  const ancestors: string[] = [];
  let current = meta.parentByKey.get(key);
  while (current) {
    if (!meta.groupKeys.has(current)) ancestors.push(current);
    current = meta.parentByKey.get(current);
  }
  return ancestors;
}

function deriveCheckedKeys(permissionCodes: Iterable<string>, meta: PermissionTreeMeta): Key[] {
  const checked = new Set(permissionCodes);
  for (const groupKey of meta.groupKeys) {
    const descendants = meta.descendantsByKey.get(groupKey) ?? [];
    const permissionDescendants = descendants.filter((key) => !meta.groupKeys.has(key));
    if (permissionDescendants.length > 0 && permissionDescendants.every((key) => checked.has(key))) {
...
function deriveHalfCheckedKeys(checkedKeys: Iterable<Key>, meta: PermissionTreeMeta): Key[] {
  const checked = new Set(Array.from(checkedKeys, String));
  const halfChecked: string[] = [];
  for (const [key, descendants] of meta.descendantsByKey) {
    if (checked.has(key)) continue;
    const permissionDescendants = descendants.filter((item) => !meta.groupKeys.has(item));
    if (permissionDescendants.length === 0) continue;
    if (permissionDescendants.some((item) => checked.has(item))) {
...
function PermissionDrawer({ open, role, onClose, onSaved }: PermissionDrawerProps) {
  ...
  const permTree     = useMemo(
    () => [
      ...buildPermissionTree(DASHBOARD_PERMISSION_ITEMS),
      ...buildPermissionTree(ALL_MENU_ITEMS),
    ],
    [],
  );
  const treeData     = useMemo(() => toAntTreeData(permTree), [permTree]);
  const treeMeta     = useMemo(() => buildPermissionTreeMeta(permTree), [permTree]);
  ...
  useEffect(() => {
    ...
    request.get<{
      ...
      .then(({ data: res }) => {
        ...
        // 严格以接口返回权限码回显；仅派生分组节点的展示勾选/半选状态，不引入本地缓存。
        const permissionCodes = existingCodes.filter((c) => !treeMeta.groupKeys.has(c));
        const nextCheckedKeys = deriveCheckedKeys(permissionCodes, treeMeta);
        setCheckedKeys(nextCheckedKeys);
        setHalfCheckedKeys(deriveHalfCheckedKeys(nextCheckedKeys, treeMeta));
      })
      .catch(() => {
  ...
  }, [open, role, expandedInit, treeMeta]);

  const handleCheck = useCallback(
    (_keys: Key[] | { checked: Key[]; halfChecked: Key[] }, info: { checked?: boolean; node: { key: Key } }) => {
      const targetKey = String(info.node.key);
      const permissionCodes = new Set(
        checkedKeys
          .map(String)
          .filter((key) => !treeMeta.groupKeys.has(key)),
      );

      if (info.checked) {
        if (treeMeta.groupKeys.has(targetKey)) {
          for (const key of treeMeta.descendantsByKey.get(targetKey) ?? []) {
            if (!treeMeta.groupKeys.has(key)) permissionCodes.add(key);
          }
        } else {
          permissionCodes.add(targetKey);
          for (const ancestor of getPermissionAncestors(targetKey, treeMeta)) {
            permissionCodes.add(ancestor);
          }
        }
      } else {
        permissionCodes.delete(targetKey);
        for (const key of treeMeta.descendantsByKey.get(targetKey) ?? []) {
          if (!treeMeta.groupKeys.has(key)) permissionCodes.delete(key);
        }
      }
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      // 只提交真正勾选的权限码；halfCheckedKeys 仅用于 UI 展示，不能当作已选权限提交。
      const permissionCodes = [...new Set(checkedKeys.map(String))]
  ...
  }, [role, checkedKeys, groupKeys, onSaved]);
