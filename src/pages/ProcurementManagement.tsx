...
import request, { isAxiosNetworkError } from '../lib/request';
import AlibabaMappingModal from '../components/AlibabaMappingModal';
import { useAlibabaAccounts } from '../hooks/useAlibabaAccounts';
import {
  alibabaAuthQueryParams,
  pickInitialAlibabaAuthId,
  resolveRecordAlibabaAuthId,
  resolveAlibabaAuthId,
  withAlibabaAuthBody,
  type AlibabaAccountFilter,
...
interface PurchaseOrder {
  ...
  /** 建单时选择的归属店铺；为空表示通用备货或历史老数据 */
  shopId?: number | null;
  shopNameSnapshot?: string | null;
  shop?: { id: number; shopName?: string | null; name?: string | null } | null;
...
interface OrderProduct {
  id:                   number;   // 采购单子项 ID（PurchaseOrderItem.id），非主产品 ID
  productId?:           number | null;  // 主产品 ID（Product.id），bind 接口必须使用此值
  pnk:                  string;
  sku:                  string | null;
  ...
  /**
   * 1688 映射同步状态：false 表示用户已更换采购链接但尚未重新同步规格，视为无效映射。
   * null/undefined 表示后端未返回该字段，按宽容策略处理（仍依赖 ID 判断）。
   */
  externalSynced?:      boolean | null;
}
...
/**
 * 从采购单子项原始 JSON 提取主产品 ID（Product.id），供 1688 bind 等接口使用。
 * 兼容多种后端命名与嵌套：productId / product_id / localProductId / product.id 等。
 */
function extractMainProductIdFromLineRaw(raw: Record<string, unknown>): number | null {
  ...
  const tryFlat: unknown[] = [
    raw.productId, raw.product_id,
    raw.localProductId, raw.local_product_id,
    raw.inventoryProductId, raw.inventory_product_id,
    raw.inventorySkuId, raw.inventory_sku_id, raw.inventorySKUId,
    raw.skuProductId, raw.sku_product_id,
  ];
  for (const v of tryFlat) {
    const n = toPositiveInt(v);
    if (n != null) return n;
  }

  const pur = raw.purchase;
  if (pur && typeof pur === 'object' && pur !== null) {
    const o = pur as Record<string, unknown>;
    const n = toPositiveInt(o.productId ?? o.product_id);
    if (n != null) return n;
  }

  // 嵌套实体（Prisma / 多 ORM 常见）：product / Product / localProduct / inventorySku / skuProduct
  const nestedBlobs: unknown[] = [
    raw.product,
  ...
  for (const blob of nestedBlobs) {
    ...
    const n = toPositiveInt(o.productId ?? o.product_id ?? o.id);
    if (n != null) return n;
  }
...
/** 子项归一化后的主产品 ID（bind 弹窗用，避免遗漏字段时仍取不到） */
function getOrderLineMainProductId(p: OrderProduct): number | null {
...
function normalizeOrderProduct(raw: Record<string, unknown>): OrderProduct {
  return {
    id: raw.id as number,
    productId: extractMainProductIdFromLineRaw(raw) ?? null,
    ...
    externalSynced: (raw.externalSynced ?? raw.external_synced ?? null) as boolean | null | undefined,
  };
}
...
function normalizePurchaseOrder(raw: Record<string, unknown>): PurchaseOrder {
  ...
  return {
    ...
    shopId:           (raw.shopId           ?? raw.shop_id           ?? null) as number | null | undefined,
    shopNameSnapshot: (raw.shopNameSnapshot ?? raw.shop_name_snapshot ?? raw.shopName ?? raw.shop_name ?? null) as string | null | undefined,
    shop:             (raw.shop             ?? null) as { id: number; shopName?: string | null; name?: string | null } | null | undefined,
    ...
    alibabaTotalAmount: (raw.alibabaTotalAmount ?? raw.alibaba_total_amount ?? null) as number | null | undefined,
    shippingFee:        (raw.shippingFee        ?? raw.shipping_fee         ?? null) as number | null | undefined,
    alibabaAuthId:      (raw.alibabaAuthId      ?? raw.alibaba_auth_id      ?? raw.alibabaAccountId ?? raw.alibaba_account_id ?? null) as number | null | undefined,
    alibabaAccountId:   (raw.alibabaAccountId   ?? raw.alibaba_account_id   ?? raw.alibabaAuthId ?? raw.alibaba_auth_id ?? null) as number | null | undefined,
    alibabaAccountName: (raw.alibabaAccountName ?? raw.alibaba_account_name ?? null) as string | null | undefined,
  };
}
...
const CANCELLED_STATUSES  = new Set(['CANCELLED']);                        // 已作废（1688 取消后走 cancel-and-release）

const ALIBABA_STATUS_MAP: Record<string, { label: string; color: string }> = {
  // 下划线格式（1688 标准）
  wait_buyer_pay:    { label: '待付款',   color: 'warning'    },
  wait_seller_send:  { label: '待发货',   color: 'processing' },
  wait_buyer_receive:{ label: '待收货',   color: 'processing' },
  seller_send:       { label: '已发货',   color: 'cyan'       },
  seller_part_send:  { label: '部分发货', color: 'processing' },
  finish:            { label: '交易完成', color: 'success'    },
  success:           { label: '交易成功', color: 'success'    },
  cancel:            { label: '交易取消', color: 'default'    },
  closed:            { label: '交易关闭', color: 'default'    },
  // 拼接格式（部分渠道/版本返回）
  waitbuyerpay:      { label: '待付款',   color: 'warning'    },
  waitsellersend:    { label: '待发货',   color: 'processing' },
  waitbuyerreceive:  { label: '待收货',   color: 'processing' },
};
...
export default function ProcurementManagement() {
  ...
  const [alibabaAccountFilter, setAlibabaAccountFilter] = useState<AlibabaAccountFilter>('ALL');
  const {
    accounts: enabledAlibabaAccounts,
    loading: alibabaAccountsLoading,
    hasAccounts: hasAlibabaAccounts,
  } = useAlibabaAccounts({ enabledOnly: true });
  const selectedAlibabaAuthId = useMemo(
    () => resolveAlibabaAuthId(alibabaAccountFilter),
    [alibabaAccountFilter],
  );
  const [expandedRowKeys, setExpandedRowKeys] = useState<(string | number)[]>([]);
  /** 列表请求失败且无 HTTP 响应（CORS/断网等），用于 Empty 文案，避免误显「暂无采购单」 */
  const [ordersNetworkError, setOrdersNetworkError] = useState(false);
  ...
  const fetchOrders = useCallback(async (
    ...
    tab: string,
    kw: string,
    accountFilter: AlibabaAccountFilter = alibabaAccountFilter,
  ) => {
    setLoading(true);
    setOrdersNetworkError(false);
    try {
      const params: Record<string, unknown> = { page: p, pageSize: ps };
      if (tab !== 'ALL') params.tabStatus = tab;
      if (kw) params.keyword = kw;
      const accountId = resolveAlibabaAuthId(accountFilter);
      if (accountId != null) params.alibabaAccountId = accountId;
  ...
  const handleBatchSync = useCallback(async (orderId: number) => {
    ...
    const authBody = selectedAlibabaAuthId != null ? { alibabaAuthId: selectedAlibabaAuthId } : {};
    try {
      ...
      try {
        await request.post(`/purchases/${orderId}/sync-logistics`, authBody);
      } catch {
        // 物流暂无属于正常情况，静默忽略
      }
      ...
      if (!isAxiosNetworkError(err)) message.error('同步失败，请检查网络');
    } finally {
      setBatchSyncingId(null);
    }
  }, [page, pageSize, fetchOrders, activeTab, keyword, selectedAlibabaAuthId]);
  ...
  const handleMarkPurchasing = useCallback(async (orderId: number) => {
    try {
      ...
      if (!isAxiosNetworkError(err)) message.error('网络异常，请重试');
    }
  }, [refresh]);
  ...
  const handleRollback = useCallback(async (orderId: number) => {
    try {
      ...
      if (!isAxiosNetworkError(err)) message.error('网络异常，撤销失败，请重试');
    }
  }, [refresh]);
  ...
  const handleForceComplete = useCallback(async (orderId: number) => {
    try {
      ...
      if (!isAxiosNetworkError(err)) message.error('网络异常，强行结单失败，请重试');
    }
  }, [refresh]);
  ...
  // 作废并释放：1688 订单取消后走严谨财务路径，保留底单并释放产品回采购计划
  const handleCancelAndRelease = useCallback(async (orderId: number) => {
    try {
      const { data: res } = await request.post<{ code: number; message: string }>(
        `/purchases/${orderId}/cancel-and-release`,
      );
      if (res.code === 200) {
        message.success(res.message || '已作废采购单，产品已释放回采购计划');
        refresh();
      } else {
        message.error(res.message || '操作失败，请重试');
      }
    } catch (err) {
      message.error(resolveAxiosError(err, '网络异常，请稍后重试', '作废失败，请重试'));
    }
  }, [refresh]);
  ...
  const columns = useMemo<ColumnsType<PurchaseOrder>>(() => [
    {
      ...
      render: (v: string, record: PurchaseOrder) => {
        const shopLabel = record.shopNameSnapshot ?? record.shop?.shopName ?? record.shop?.name ?? null;
        return (
          <div style={{ lineHeight: 1.6 }}>
            <Text strong style={{ fontFamily: "'Inter', monospace", fontSize: 13, letterSpacing: 0.3 }}>{v}</Text>
            <div style={{ marginTop: 4 }}>
              {shopLabel
                ? <Tag color="blue" style={{ margin: 0 }}>{shopLabel}</Tag>
                : <Tag color="default" style={{ margin: 0 }}>通用备货</Tag>}
            </div>
          </div>
        );
      },
    },
    {
      ...
      title: '供应商 / 1688订单', key: 'supplierOrder', width: 260,
      render: (_: unknown, record: PurchaseOrder) => (
        <div style={{ lineHeight: '1.6', fontSize: 12 }}>
          {record.alibabaAccountName ? (
            <div style={{ marginBottom: 2 }}>
              <Tag color="orange" style={{ margin: 0, fontSize: 11 }}>{record.alibabaAccountName}</Tag>
            </div>
          ) : null}
    ...
    {
      title: '总采购金额', key: 'totalAmount', width: 145, align: 'right' as const,
      render: (_: unknown, record: PurchaseOrder) => {
        const refAmt = fmtMoney(record.totalAmount);
        const aliAmt = record.alibabaTotalAmount;
        const aliFee = record.shippingFee;

        // 未同步 1688：只显示本地参考金额（保持原样式）
        if (!aliAmt) {
          return (
            <span style={{ fontWeight: 700, fontSize: 14, color: refAmt === '-' ? undefined : '#d4380d', fontFeatureSettings: '"tnum"' }}>
              {refAmt}
            </span>
          );
        }

        // 已同步 1688：第一行实际金额+运费，第二行参考金额
        return (
          <div style={{ lineHeight: 1.8 }}>
            <div>
              ...
              {aliFee != null && aliFee > 0 && (
                <span style={{ fontSize: 11, color: '#8c8c8c', marginLeft: 6 }}>
                  运费 {fmtMoney(aliFee)}
                </span>
              )}
            </div>
            <div style={{ fontSize: 13, fontWeight: 400, color: '#1890ff', fontFeatureSettings: '"tnum"', marginTop: 2 }}>
              参考金额: {refAmt}
            </div>
          </div>
        );
          ...
          <Space size={4} wrap>
            ...
            {isCancelled && (
              <Popconfirm
                title="确认退回重新下单？"
                description={
                  <span style={{ maxWidth: 280, display: 'inline-block', lineHeight: 1.6 }}>
                    当前 1688 订单已取消。点击确认将<strong>作废</strong>此采购单以保留财务底单，
                    并将产品释放回「采购计划」。您需要前往采购计划重新建单。
                  </span>
                }
                okText="确认退回重下"
                cancelText="取消"
                okButtonProps={{ style: { background: '#fa8c16', borderColor: '#fa8c16' } }}
                onConfirm={() => handleCancelAndRelease(record.id)}
              >
                <Button
                  size="small"
  ...
  ], [handleBatchSync, batchSyncingId, setPlace1688Target, handleMarkPurchasing, setStockInTarget, handleRollback, handleDeleteOrder, warehouseOptions, handlePatchWarehouse, setLogisticsEditTarget, setLogisticsTraceTarget, handleCancelAndRelease]);

  // ── 嵌套子表 ──

  const handleAlibabaAccountFilterChange = useCallback((val: number | 'ALL') => {
    setAlibabaAccountFilter(val);
    setPage(1);
    setExpandedRowKeys([]);
    fetchOrders(1, pageSize, activeTab, keyword, val);
  }, [fetchOrders, pageSize, activeTab, keyword]);
  ...
  const expandedRowRender = useCallback((record: PurchaseOrder) => (
    <OrderProductsTable
      ...
      alibabaAuthId={selectedAlibabaAuthId}
      onOpenLogistics={handleOpenLogistics}
      onSyncSuccess={() => setSubRefreshKey((k) => k + 1)}
    />
  ), [subRefreshKey, handleOpenLogistics, selectedAlibabaAuthId]);
  ...
  return (
    <div className="min-h-full">
      ...
      {!alibabaAccountsLoading && !hasAlibabaAccounts && (
        <Alert
          type="warning"
          showIcon
          style={{ marginBottom: 12 }}
          message="请先到 系统设置 / 1688 配置 添加 1688 账号"
          action={
            <Button size="small" type="link" onClick={() => navigate('/dashboard?tab=alibaba-settings')}>
              去配置
            </Button>
          }
        />
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100" style={{ overflow: 'hidden' }}>
        ...
        {/* ── 工具栏：搜索 + 展开/收起 ── */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderBottom: '1px solid #f5f5f5', flexWrap: 'wrap' }}>
          <Button
            size="small"
          ...
          <Space size={4}>
            <Text type="secondary" style={{ fontSize: 12 }}>1688 账号</Text>
            <Select
              size="small"
              style={{ width: 180 }}
              value={alibabaAccountFilter}
              loading={alibabaAccountsLoading}
              onChange={(val) => handleAlibabaAccountFilterChange(val as number | 'ALL')}
              options={[
                { value: 'ALL', label: '全部账号' },
                ...enabledAlibabaAccounts.map((a) => ({
                  value: a.id,
                  label: a.isDefault ? `${a.name}（默认）` : a.name,
                })),
              ]}
            />
          </Space>
          ...
          locale={{
            emptyText: (
              <Empty
                ...
                description={ordersNetworkError ? '网络连接异常，无法加载采购单' : '暂无采购单'}
...
interface SpecSelectModalProps {
  ...
  alibabaAuthId?: number;
  onCancel:  () => void;
  onSuccess: (productId: number, specId: string) => void;
}

function SpecSelectModal({ product, alibabaAuthId, onCancel, onSuccess }: SpecSelectModalProps) {
    ...
    (async () => {
      try {
        const { data: res } = await request.get<{
          ...
          message?: string;
        }>('/alibaba/product-specs', { params: { offerId, ...alibabaAuthQueryParams(alibabaAuthId) } });
        ...
        if (!isAxiosNetworkError(err)) message.error('获取规格失败，请检查网络');
        setSpecs([]);
      } finally {
        setSpecsLoading(false);
      }
    })();
  }, [product, alibabaAuthId]);
...
function LogisticsTraceModal({ record, alibabaAuthId, onCancel }: LogisticsTraceModalProps) {
    ...
    (async () => {
      try {
        const { data: res } = await request.get<{ code: number; data?: unknown; message?: string }>(
          ...
          { params: alibabaAuthQueryParams(alibabaAuthId) },
        );
        if (cancelled) return;
        if (!res || res.code !== 200) {
...
function LogisticsDrawer({ open, externalOrderId, alibabaAuthId, onClose }: LogisticsDrawerProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data: res } = await request.get<{ code: number; data?: LogisticsData; message?: string }>(
          ...
          { params: { externalOrderId, ...alibabaAuthQueryParams(alibabaAuthId) } },
        );
    ...
    return () => { cancelled = true; };
  }, [open, externalOrderId, alibabaAuthId]);
...
function OrderProductsTable({ orderId, refreshKey = 0, alibabaAuthId, onOpenLogistics, onSyncSuccess }: OrderProductsTableProps) {
  const [products, setProducts] = useState<OrderProduct[]>([]);
  const [loading, setLoading] = useState(true);
  const [syncingAlibabaId, setSyncingAlibabaId] = useState<string | null>(null);
  /** 明细请求为网络层失败时置 true，Empty 显示「网络连接异常」而非「暂无」 */
  const [subTableNetworkError, setSubTableNetworkError] = useState(false);
  ...
  const handleSyncRow = useCallback(async (externalOrderId: string) => {
    ...
    try {
      const { data: res } = await request.post<{ code: number; message?: string }>(
        ...
        withAlibabaAuthBody({ externalOrderId }, alibabaAuthId),
      );
      if (res?.code === 200) {
  ...
  }, [fetchProducts, onSyncSuccess, alibabaAuthId]);
  ...
  const subColumns = useMemo<ColumnsType<OrderProduct>>(() => [
    {
      ...
      render: (url: string | null) =>
        url
          ? <Image src={url} width={36} height={36} referrerPolicy="no-referrer" style={{ objectFit: 'cover', borderRadius: 4, border: '1px solid #f0f0f0' }} preview={{ mask: <SearchOutlined style={{ fontSize: 9 }} /> }} />
    ...
    <Table
      ...
      locale={{
        emptyText: (
          <Empty
            image={Empty.PRESENTED_IMAGE_SIMPLE}
            description={subTableNetworkError && !loading ? '网络连接异常，无法加载明细' : '暂无明细'}
...
interface Place1688OrderModalProps {
  ...
  onCancel:      () => void;
  onSuccess:     () => void;
  /** 外部数据刷新信号：父组件 subRefreshKey 每次变化时，弹窗内产品明细自动静默同步 */
  refreshCount?: number;
  /** 当前选中的 1688 账号；未选则后端使用默认账号 */
  alibabaAuthId?: number;
}
 ...
 *   'stale'   → ID 与 specId 均存在，但 externalSynced === false（换链后尚未重新同步）
 *   'ok'      → 商品 ID + specId 均具备且同步有效（可下单）
 */
function getMappingStatus(p: OrderProduct): 'none' | 'no_sku' | 'stale' | 'ok' {
  ...
  // externalSynced 明确为 false 时认定为已失效；null/undefined 按宽容策略放行
  if (p.externalSynced === false) return 'stale';
  return 'ok';
}

/** 判断单个产品是否已完成 1688 完整映射且同步有效（可下单） */
...
/**
 * 统一解析 axios 错误：
 * - 网络层失败（无 response）→ 使用 networkFallback 文案
 * - 有 HTTP 响应（4xx/5xx）  → 优先取 response.data.message；5xx 加 [服务端错误] 前缀
 * - 兜底 → bizFallback 文案
 */
function resolveAxiosError(
  err: unknown,
  networkFallback: string,
  bizFallback: string,
): string {
  if (isAxiosNetworkError(err)) return networkFallback;
  const axiosErr = err as { response?: { status?: number; data?: { message?: string; code?: number } } };
  const body     = axiosErr?.response?.data;
  const hint     = body?.message ?? bizFallback;
  const httpStatus = axiosErr?.response?.status ?? 0;
  const bodyCode   = body?.code ?? 0;
  const isServer   = httpStatus >= 500 || bodyCode >= 500;
  return isServer ? `[服务端错误] ${hint}` : hint;
}

function Place1688OrderModal({ record, onCancel, onSuccess, refreshCount = 0, alibabaAuthId: filterAuthId }: Place1688OrderModalProps) {
  const {
    accounts: enabledAlibabaAccounts,
    loading: alibabaAccountsLoading,
    defaultAccount,
    hasEnabledAccounts,
    reload: reloadAlibabaAccounts,
  } = useAlibabaAccounts({ enabledOnly: true, autoLoad: false });
  ...
  /** 产品明细请求为网络层失败（用于 Empty 文案与提交前校验） */
  const [productsNetworkError, setProductsNetworkError] = useState(false);
  // 本次下单中临时排除的产品 ID（不影响原单据）
  const [excludedIds,   setExcludedIds]   = useState<Set<number>>(new Set());
  // 规格补全子弹窗：当前待补全 specId 的产品（仅用于 no_sku 场景）
  const [specEditTarget, setSpecEditTarget] = useState<OrderProduct | null>(null);
  // 换链重绑子弹窗：当前需重新关联 1688 的产品（stale 场景）
  const [rebindTarget, setRebindTarget] = useState<OrderProduct | null>(null);

  // ── Tab 2 状态（手动关联） ──
  const [bindForm]    = Form.useForm<{ alibabaOrderId: string }>();
  const [bindSubmitting, setBindSubmitting] = useState(false);

  // 防止初次挂载时与正常 record 加载重复触发 reload
  const prevRefreshCountRef = useRef<number>(refreshCount);

  /**
   * 健壮化产品明细拉取：兼容后端 4 种返回结构（直接数组 / list / items / products）。
   * 返回最新 OrderProduct[] 供下单前二次校验使用，不再依赖 state 时序。
   */
  const reloadProducts = useCallback(async (): Promise<{ list: OrderProduct[]; networkError: boolean }> => {
    if (!record) return { list: [], networkError: false };
    setProductsLoading(true);
    setProductsNetworkError(false);
    try {
      const { data: res } = await request.get<{ code: number; data?: unknown; message?: string }>(
        purchaseOrderProductsUrl(record.id),
      );
      if (!res || res.code !== 200) {
        message.warning(res?.message || '采购明细加载失败，请稍后重试');
        setProducts([]);
        return { list: [], networkError: false };
      }
      const payload = res?.data;
      let rawList: unknown[] = [];
      if (Array.isArray(payload)) {
      ...
      return { list: normalized, networkError: false };
    } catch (err) {
      const ne = isAxiosNetworkError(err);
      setProductsNetworkError(ne);
      setProducts([]);
      return { list: [], networkError: ne };
    } finally {
      setProductsLoading(false);
    }
  }, [record]);

  // 每次弹窗打开时：重置状态、初始化账号、拉取产品
  useEffect(() => {
    if (!record) {
    ...
    bindForm.resetFields();
    setExcludedIds(new Set());
    reloadAlibabaAccounts();
    reloadProducts();
  }, [record, addrForm, bindForm, reloadProducts, reloadAlibabaAccounts]);

  useEffect(() => {
    if (!record || alibabaAccountsLoading) return;
    const recordAuthId = resolveRecordAlibabaAuthId(record);
    setSelectedAccountId(pickInitialAlibabaAuthId({
      recordAuthId,
      filterAuthId,
      defaultAccountId: defaultAccount?.id,
      firstEnabledAccountId: enabledAlibabaAccounts[0]?.id,
    }));
  }, [record, filterAuthId, alibabaAccountsLoading, defaultAccount?.id, enabledAlibabaAccounts]);

  useEffect(() => {
    if (!record || selectedAccountId == null) {
      setAddresses([]);
      return;
    }
    setAddrLoading(true);
    request.get<{ code: number; data: AliAddress[]; message: string }>('/alibaba/addresses', {
      params: alibabaAuthQueryParams(selectedAccountId),
    })
      .then(({ data: res }) => {
      ...
      .catch((err) => {
        setAddresses([]);
        if (!isAxiosNetworkError(err)) message.error('获取收货地址失败，请检查网络');
      })
      .finally(() => setAddrLoading(false));
  }, [record, selectedAccountId, addrForm]);

  // 监听外部换链/关联刷新信号：refreshCount 变化时自动同步弹窗内产品数据
  useEffect(() => {
    if (!record) return;
    if (prevRefreshCountRef.current === refreshCount) return;
    prevRefreshCountRef.current = refreshCount;
    reloadProducts();
  }, [record, refreshCount, reloadProducts]);
  ...
  const canSubmit = !hasUnmapped && activeProducts.length > 0 && selectedAccountId != null && hasEnabledAccounts;

  // Tab 1：自动下单（含提交前静默刷新，确保 externalSkuId 使用最新值）
  const handleAutoSubmit = async () => {
    if (!hasEnabledAccounts || selectedAccountId == null) {
      message.error('请先选择 1688 采购账号');
      return;
    }
    ...
    try {
      // ① 静默刷新产品明细：防止换链后 externalSkuId 仍为缓存旧值
      const { list: latestProducts, networkError: reloadNetErr } = await reloadProducts();
      if (reloadNetErr) {
        message.warning('网络连接异常，无法完成下单前校验，请稍后重试');
        return;
      }
      const activeLatest   = latestProducts.filter((p) => !excludedIds.has(p.id));
      const unmappedLatest = activeLatest.filter((p) => !isProductMapped(p));

      if (activeLatest.length === 0) {
        message.warning('没有待下单的产品，请确认是否全部被移除');
        return;
      }
      if (unmappedLatest.length > 0) {
        const staleCount = unmappedLatest.filter((p) => getMappingStatus(p) === 'stale').length;
        const hint = staleCount > 0
          ? `${staleCount} 个产品的采购链接已更换且映射失效，请重新同步规格后再提交`
          : `${unmappedLatest.length} 个产品 1688 规格（specId）尚未补全，请先完成映射后再提交`;
      ...
      message.error(resolveAxiosError(err, '网络异常，下单失败，请稍后重试', '下单失败，请重试'));
    }
  ...
  const handleBindSubmit = async () => {
    ...
    try {
      const { data: res } = await request.post<{ code: number; message: string }>(
        ...
        withAlibabaAuthBody({ alibabaOrderId: values.alibabaOrderId.trim() }, selectedAccountId),
      );
      if (res.code === 200) { message.success(res.message || '1688 订单关联成功！'); onSuccess(); }
      else { message.error(res.message || '关联失败，请检查单号后重试'); }
    } catch (err) {
      message.error(resolveAxiosError(err, '网络异常，关联失败，请稍后重试', '关联失败，请检查单号后重试'));
    }
  ...
  // 1688 换链重绑成功：先局部更新当前 SKU 映射状态，让 canSubmit 立即重算；
  // 随后再刷新后端明细，避免用户必须关闭主弹窗重开才能下单。
  const handleRebindDone = useCallback(async (payload?: {
    action: 'bind' | 'unbind';
    productId: number;
    offerId?: string;
    specId?: string;
    ...
    const markMapped = (rows: OrderProduct[]) =>
      rows.map((p) => (
        p.productId === payload.productId
          ? {
              ...p,
              alibabaOfferId: payload.offerId ?? p.alibabaOfferId,
              externalProductId: payload.offerId ?? p.externalProductId,
              externalSkuId: payload.specId ?? payload.skuId ?? p.externalSkuId,
              externalSynced: true,
            }
          : p
      ));

    setProducts((prev) => markMapped(prev));

    const { list } = await reloadProducts();
    const refreshedOk = list.some((p) =>
      p.productId === payload.productId && isProductMapped(p),
    );
    if (!refreshedOk) {
      setProducts((prev) => markMapped(prev));
    }
  ...
  const mappingColumns: ColumnsType<OrderProduct> = [
    ...
    },
    {
      ...
      render: (_: unknown, p: OrderProduct) => {
        ...
        if (status === 'stale') {
          const missingProductId = getOrderLineMainProductId(p) == null;
          return (
            <div>
              <Tag color="orange" style={{ fontSize: 11, marginBottom: 4, fontWeight: 600 }}>
                采购链接已更换，请重新同步映射
              </Tag>
              <div style={{ fontSize: 10, color: '#d46b08', lineHeight: 1.5, marginBottom: 6 }}>
                旧映射已失效，需重新关联 1688 规格后方可下单
              </div>
              <Tooltip title={missingProductId ? '缺少产品主键，无法绑定' : '重新同步 1688 规格'}>
                <span style={{ display: 'inline-block' }}>
                  <Button
                    type="link"
                    size="small"
                    icon={<LinkOutlined />}
                    disabled={missingProductId}
                    style={{
                      padding: 0,
                      fontSize: 11,
                      color: missingProductId ? undefined : '#fa8c16',
                      fontWeight: 600,
                    }}
                    onClick={() => {
                      if (missingProductId) return;
                      setRebindTarget(p);
                    }}
                  >
                    重新绑定
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
      ...
      {activeTabKey === 'auto' ? (
        <Tooltip
          title={
            !canSubmit
              ? (selectedAccountId == null || !hasEnabledAccounts
                ? '请先选择 1688 采购账号'
                : hasUnmapped
                  ? '存在未映射产品，请移除或先完成映射'
                  : '没有待下单产品')
        ...
        <Button type="primary" loading={bindSubmitting} disabled={!hasEnabledAccounts || selectedAccountId == null} onClick={handleBindSubmit}>
          确认关联
        </Button>
      )}
    </div>
  ...
  return (
    ...
    >
      {!alibabaAccountsLoading && !hasEnabledAccounts && (
        <Alert
          type="warning"
          showIcon
          style={{ marginBottom: 12 }}
      ...
      <Form layout="vertical" style={{ marginBottom: 12 }}>
        <Form.Item
          label={<span style={{ fontWeight: 600, fontSize: 13 }}>1688 采购账号</span>}
          style={{ marginBottom: 0 }}
          help={
            record?.alibabaAccountName
              ? `采购单已绑定：${record.alibabaAccountName}`
              : '未绑定时默认选中系统默认账号，可在此切换'
          }
        >
          {alibabaAccountsLoading ? (
            <Spin size="small" />
          ) : (
            <Select
              placeholder="请选择 1688 采购账号"
              value={selectedAccountId}
              onChange={setSelectedAccountId}
              style={{ width: '100%' }}
              disabled={!hasEnabledAccounts}
              options={enabledAlibabaAccounts.map((a) => ({
                value: a.id,
                label: a.isDefault ? `${a.name}（默认）` : a.name,
              }))}
            />
                  ...
                  <Table<OrderProduct>
                    ...
                    locale={{
                      emptyText: (
                        <Empty
                          image={Empty.PRESENTED_IMAGE_SIMPLE}
                          description={productsNetworkError && !productsLoading ? '网络连接异常，无法加载产品' : '暂无产品数据'}
                        />
                      ),
                    }}
                  />
      ...
      <SpecSelectModal
        ...
        alibabaAuthId={selectedAccountId}
        onCancel={() => setSpecEditTarget(null)}
        onSuccess={handleSpecMapped}
      />

      {/* 换链重绑子弹窗：stale 状态下直接唤起全量 1688 绑定流程 */}
      <AlibabaMappingModal
        open={!!rebindTarget}
        productId={rebindTarget?.productId ?? null}
        productSku={rebindTarget?.sku ?? null}
        purchaseUrl={rebindTarget?.purchaseUrl ?? null}
        currentOfferId={rebindTarget?.alibabaOfferId ?? rebindTarget?.externalProductId ?? null}
        currentSpecId={rebindTarget?.externalSkuId ?? null}
...
function StockInModal({ record, onCancel, onSuccess, prefetchedItems }: StockInModalProps) {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      if (!isAxiosNetworkError(err)) message.error('网络异常，入库失败，请重试');
    }
  ...
  const receiveColumns: ColumnsType<ReceiveRow> = [
    {
      ...
      render: (url: string | null) => url
        ? <Image src={url} width={36} height={36} referrerPolicy="no-referrer" style={{ objectFit: 'cover', borderRadius: 4, border: '1px solid #f0f0f0' }} preview={false} />
