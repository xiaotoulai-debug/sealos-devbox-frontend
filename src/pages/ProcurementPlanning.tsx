...
import {
  ...
  InputNumber, Input, Dropdown, Form, Select, Spin, Alert,
} from 'antd';
...
export default function ProcurementPlanning() {
  ...
  const inferredDefaultShopId = useMemo(() => {
    if (selectedRows.length === 0) return undefined;
    const firstShopId = selectedRows[0].shopId ?? selectedRows[0].shop_id ?? undefined;
    if (firstShopId == null) return undefined;
    return selectedRows.every((row) => (row.shopId ?? row.shop_id ?? undefined) === firstShopId)
      ? firstShopId
      : undefined;
  }, [selectedRows]);
      ...
      if (res.code === 200 && res.data) {
        setProducts(
          Array.isArray(res.data.list)
            ? res.data.list.map((item) => normalizePurchasingPlanProduct(item as Record<string, unknown>))
            : [],
        );
  ...
  const fbeBatchBlockReason = useMemo(
    () => (selectedRows.length > 0 ? getFbeBatchBlockReason(selectedRows) : null),
    [selectedRows],
  );

  const handleOpenFbeModal = useCallback(() => {
    const reason = getFbeBatchBlockReason(selectedRows);
    if (reason) {
  ...
  const dropdownItems = useMemo<MenuProps['items']>(() => [
    ...
    },
    {
      ...
      icon:     <SendOutlined />,
      label:    '📦 创建 FBE 发货单',
      disabled: selectedRows.length === 0 || !!getFbeBatchBlockReason(selectedRows),
      onClick:  handleOpenFbeModal,
    },
    {
      key:   'download',
  ...
  ], [handleBatchDownload, handleBatchRemove, handleOpenFbeModal, selectedRows]);
  ...
  const columns = useMemo<ColumnsType<PurchasingProduct>>(() => [
    ...
    {
      title: 'FBE', key: 'fbe', width: 120, align: 'center',
      render: (_: unknown, record: PurchasingProduct) => {
        const shipmentId = resolveFbeShipmentId(record);
        if (shipmentId != null) {
          return (
            <Tooltip title={record.fbeShipmentNo ?? record.fbe_shipment_no ?? `发货单 #${shipmentId}`}>
              <Tag color="blue" bordered={false} style={{ borderRadius: 6, fontSize: 11 }}>
                已创建 FBE
              </Tag>
            </Tooltip>
          );
  ...
  return (
    <div className="min-h-full">
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        >
          <Tooltip title={fbeBatchBlockReason ?? undefined}>
            <Button
              type="primary"
              icon={<ToolOutlined />}
              disabled={selectedRowKeys.length === 0}
      ...
      <OrderConfirmModal
        open={orderModalOpen}
        rows={selectedRows}
        initialShopId={inferredDefaultShopId}
      ...
      <CreateFbeFromPurchasePlansModal
        open={fbeModalOpen}
        products={selectedRows}
        onCancel={() => setFbeModalOpen(false)}
        onSuccess={(shipmentId) => {
          setFbeModalOpen(false);
          setSelectedRowKeys([]);
          setSelectedRows([]);
          setPage(1);
          fetchProducts(1, pageSize);
          if (shipmentId != null) {
            navigate(`/dashboard?tab=fbe-shipments&fbeShipmentId=${shipmentId}`);
          } else {
            navigate('/dashboard?tab=fbe-shipments');
          }
        }}
      />
...
function OrderConfirmModal({ open, rows, initialShopId, onCancel, onSuccess }: OrderConfirmModalProps) {
  const navigate = useNavigate();
  const {
  ...
  const [alibabaAuthId, setAlibabaAuthId] = useState<number | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    request.get<{ code: number; data?: ShopOption[] | { list?: ShopOption[] } }>(
      '/shops',
    ).then(({ data: res }) => {
      if (res.code === 200) {
        const raw = res.data;
  ...
  }, [open, rows, initialShopId, reloadAlibabaAccounts]);

  useEffect(() => {
    if (!open) return;
    if (alibabaAccountsLoading) return;
    const defaultId = defaultAccount?.id ?? enabledAlibabaAccounts[0]?.id;
    setAlibabaAuthId(defaultId);
  }, [open, alibabaAccountsLoading, defaultAccount?.id, enabledAlibabaAccounts]);
  ...
  const handleConfirm = async () => {
    ...
    if (!hasEnabledAccounts || alibabaAuthId == null) {
    ...
    try {
      ...
      if (isAxiosNetworkError(err)) {
        ...
        const data = (err as { response?: { data?: { message?: string; code?: number } } })?.response?.data;
        const hint = data?.message || '创建采购单失败';
        const prefix = data?.code != null && data.code >= 500 ? '[服务端错误] ' : '';
        message.error(`${prefix}${hint}`);
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const orderColumns = useMemo<ColumnsType<PurchasingProduct>>(() => [
    {
      ...
      render: (url: string | null) =>
        url
          ? <Image src={url} width={40} height={40} referrerPolicy="no-referrer" style={{ objectFit: 'cover', borderRadius: 6, border: '1px solid #f0f0f0' }} preview={false} />
  ...
  return (
    <Modal
      ...
      footer={[
        ...
        <Button
          ...
          type="primary"
          loading={submitting}
          disabled={!warehouseId || !hasEnabledAccounts || alibabaAuthId == null}
          onClick={handleConfirm}
          style={warehouseId && alibabaAuthId ? { background: '#52c41a', borderColor: '#52c41a' } : {}}
        >
          确认创建采购单
        </Button>,
      ]}
    >
      ...
      <Form layout="vertical" style={{ marginBottom: 12 }}>
        <Form.Item
          label={<span style={{ fontWeight: 600, fontSize: 13 }}>1688 采购账号<span style={{ color: '#ff4d4f', marginLeft: 4 }}>*</span></span>}
          style={{ marginBottom: 12 }}
          validateStatus={!alibabaAuthId && hasEnabledAccounts ? 'warning' : 'success'}
        >
          {alibabaAccountsLoading ? (
            <div style={{ padding: '8px 0' }}><Spin size="small" /><span style={{ marginLeft: 8, color: '#9ca3af', fontSize: 12 }}>加载 1688 账号…</span></div>
          ) : (
            <Select
              placeholder="请选择 1688 采购账号"
              value={alibabaAuthId}
              onChange={setAlibabaAuthId}
              style={{ width: '100%' }}
              size="large"
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <Form.Item
            label={<span style={{ fontWeight: 600, fontSize: 13 }}>🏬 归属店铺（可选）</span>}
            style={{ marginBottom: 0 }}
            help="不选择则按通用备货处理"
          >
            {shopsLoading ? (
              <div style={{ padding: '8px 0' }}><Spin size="small" /><span style={{ marginLeft: 8, color: '#9ca3af', fontSize: 12 }}>加载店铺列表…</span></div>
            ) : (
              <Select
                placeholder="请选择归属店铺（可选）"
                value={shopId}
                onChange={setShopId}
                style={{ width: '100%' }}
                size="large"
                options={shops.map((s) => {
                  ...
                  const label = region ? `${s.shopName} (${s.platform ?? '店铺'} · ${region})` : `${s.shopName} (${s.platform ?? '店铺'})`;
