  ...
  const columns = useMemo<ColumnsType<InventoryProduct>>(() => [
    {
      ...
      render: (url: string | null) => url ? (
        <Image key={url} src={url} width={50} height={50}
        ...
        onDone={(updated) => {
          if (updated) {
            setProducts((prev) => prev.map((p) => (
              p.id === updated.id ? { ...p, imageUrl: updated.imageUrl } : p
            )));
          }
...
interface EditSKUOptimisticUpdate {
  id:       number;
  imageUrl: string | null;
}
...
interface EditSKUModalProps {
  ...
  onCancel: () => void;
  onDone: (updated?: EditSKUOptimisticUpdate) => void;
}

interface InventoryBatchUpdateResponse {
  code: number;
  message?: string;
  data?: {
    count?: number;
...
function EditSKUModal({ open, product, onCancel, onDone }: EditSKUModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const nextImageUrl = imgUrl.trim() || null;
      const { data: res } = await request.put<InventoryBatchUpdateResponse>('/products/inventory-batch-update', {
        items: [{
          id: product.id,
          sku: sku.trim(),
      ...
      const affectedCount = res.data?.count ?? res.data?.updated ?? res.count ?? res.updated ?? null;
      if (affectedCount === 0) {
        message.warning('保存未生效，请检查该 SKU 是否允许被你修改');
        return;
      }

      message.success(res.message || '保存成功');
      onDone({ id: product.id, imageUrl: nextImageUrl });
    } catch (err: unknown) {
